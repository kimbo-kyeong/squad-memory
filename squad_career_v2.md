...
- **[2026-07-08 신규] 유형그룹별 버전 2종** (4챕터 대상 6트랙을 2D/5D 그룹 + 나머지로 분리, 유형별 차등 윈도우):
  - `커리어_v2_스쿼드_weekly_그룹별.sql` (3섹터 + **유형그룹 × 과목 2차원**, MAU 제외)
  - `커리어_지표한판_weekly_그룹별.sql` (16지표 + 유형그룹 1차원, 과목 세분화·MAU제외 미적용 — 원하면 동일 적용)
  - 구조(v2): 1행 = 1주차 × **과목**(대상 6과정 각각=과정명전체 + '그 외'). "유형그룹"(2D/5D/그외)·"과목" 컬럼을 주시작일 뒤 삽입, 시트에서 유형그룹으로 접으면 2D/5D 소계. 지표 순서는 원본 동일하되 **MAU(4주롤링) 제외**(Monthly리텐션은 유지). subject_dim CTE(tgt_map + '그 외')로 과목 스파인 구성.
