...
- **[2026-07-08 신규] 유형그룹별 버전 2종** (4챕터 대상 6트랙을 2D/5D 그룹 + 나머지로 분리, 유형별 차등 윈도우):
  ...
  - `커리어_취업솔루션_코호트정착_목표추적.sql` = **[목표 추적]** 유형그룹 × 과목 × 수료주. **★목표선 = 정착 W1~4평균(2D34/5D36) + WoW(53, 유형2D/5D단위)**. 보조(목표선無) = 초기진입%[-7~+14]·정착 W+1~4주별·지속 W+5~8평균 (유입/이탈 원인규명용). 관측상태(정착확정=수료+4주). 모수=대상6과정 수료실업자(방문필터無)·테스트제외. 대상 미래수료라 현재 0행. **목표·베이스라인·레버 상세는 [[squad_career_employment]] ⑤ (2026-07-10 정착+WoW 확정).**
  - `커리어_지표한판_weekly_그룹별.sql` (16지표 + 유형그룹 1차원). 과목·MAU 미적용.
  - **테스트 제외 (2026-07-09, 3쿼리 공통)** : mart_crr_graduate JOIN public.dbonline_v2_users u ON u._id = g.user_id AND u.is_test IS NOT TRUE. (더 철저히는 name/email/하드코딩4건도 있으나 요청 범위는 is_test만 — [[user_table_join]])
  - **취업자 처리 (2026-07-10)** : 취업대상자 컬럼명 '(누적)'→'(미취업)'. weekly=주차별 시점 미취업(hired IS NULL OR hired>주차+6), 코호트=수료시점 미취업(hired>수료일)으로 분모 산정. **단 취업률 분모(난항/순항/꿀알바_모수)는 전체 누적 유지**(취업률=취업/전체모수라 취업자 포함 필수) → weekly_active_pool에 total_pool_size 별도 추가. ※ 코호트는 수료시점 고정이라 수료후 4주내 취업자는 잔류(정착기간 취업자 극소, 영향 미미). 2026-04-27 '누적' 결정은 폐기.
