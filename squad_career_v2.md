...
- **[2026-07-08 신규] 유형그룹별 버전 2종** (4챕터 대상 6트랙을 2D/5D 그룹 + 나머지로 분리, 유형별 차등 윈도우):
  ...
  - `커리어_취업솔루션_코호트정착_목표추적.sql` = **[목표 추적]** 유형그룹 × 과목 × 수료주 × W+1~W+4 활성률 + 정착1~4평균 + 목표선(2D34/5D36 CASE). 모수=대상6과정 수료실업자(방문필터無=코호트전체), 활성=crr_view, W+0 제외. 대상 미래수료라 현재 0행, 수료 시작 시 채워짐. 로직=§13/§4 동일.
