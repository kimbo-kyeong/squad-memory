...
- **[2026-07-08 신규] 유형그룹별 버전 2종** (4챕터 대상 6트랙을 2D/5D 그룹 + 나머지로 분리, 유형별 차등 윈도우):
  ...
  - **[신규] KPI 요약 뷰 2쿼리 (2026-07-10)** : `커리어_취업솔루션_KPI_유형별.sql`(1행=2D/5D/10D/그외) · `커리어_취업솔루션_KPI_트랙별.sql`(1행=과정). 연말+4챕터 KPI 현재값+목표를 요약 1행으로(시계열 아님). 지표정의·기능매핑·데이터매핑은 [[squad_career_employment]] ⑥. DB지표(수료인원·이력서/포폴 등록률·코칭사용률) 검증완료, 활동/기능(amplitude 최근4주)은 read-only로 시트/psql 실행.
